...
app.post('/api/posts', async(req, res) => {
    try {
        ...
        const newpost = await pool.query(
            "INSERT INTO posttable(body, posttime, postauthor) values ($1, $2, $3)    RETURNING*", [post.body, post.posttime, post.postauthor]
...
app.put('/api/posts/:id', async(req, res) => {
    try {
        ...
        const updatepost = await pool.query(
            "UPDATE posttable SET (body, posttime, postauthor) = ($2, $3, $4) WHERE id = $1", [id, post.body, post.posttime, post.postauthor]
        );
        res.json(updatepost);
    } catch (err) {
        console.error(err.message);
